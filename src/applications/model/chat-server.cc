...
TypeId ChatServer::GetTypeId (void){
	static TypeId tid = TypeId ("ns3::ChatServer")
		...
		.AddAttribute("Port", "Serve Port", UintegerValue(0), MakeUintegerAccessor(&ChatServer::m_port), MakeUintegerChecker<uint16_t> ())
...
void ChatServer::StartApplication(void)
{
	...
	i=0;
	while(i<100){
		r_socket[i]->SetAcceptCallback (MakeNullCallback<bool, Ptr<Socket>, const Address&> (), MakeCallback(&ChatServer::onAccept, this));
	}
...
void ChatServer::onAccept(Ptr<Socket> s, const Address& from){
	...
	/*TypeId tid = TypeId::LookupByName("ns3::TcpSocketFactory");
	  ...
	  r_socket.back()->SetAcceptCallback (MakeNullCallback<bool, Ptr<Socket>, const Address&> (), MakeCallback(&ChatServer::onAccept, this)); 
...
void ChatServer::SendPacket(void){
	...
	std::cout<< r_socket.size()<<d_to_send[0]<<" Send "<<d_to_send.size()<< "\n";
	ScheduleTx(Seconds(1.0));
}
